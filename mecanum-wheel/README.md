# EV3 Mecanum Robot via ESP32 + Bluepad32 + Pybricks

LEGO EV3 메카넘 휠 로봇을 Xbox 컨트롤러로 무선 제어하는 프로젝트입니다.  
...
로봇의 전진 방향을 기준으로 모터를 아래처럼 연결합니다.

| 위치 | EV3 포트 | 이미지 라벨 |
|------|----------|-------------|
| 앞왼쪽 Front Left | Port D | D |
| 앞오른쪽 Front Right | Port B | B |
| 뒤왼쪽 Rear Left | Port C | C |
| 뒤오른쪽 Rear Right | Port A | A |

> ℹ️ 현재 `ev3_control.py`는 위 배치를 기준으로 메카넘 믹싱을 수행합니다.
...
메카넘 휠 기준으로 ESP32는 아래 3개 값을 EV3로 전송합니다.

- `X`: 좌우 평행이동
- `Y`: 전진 / 후진
- `R`: 회전

EV3는 이 값을 사용해 4개 모터 속도 `A/B/C/D`로 변환합니다.
...
├── README.md                             # 이 파일
├── esp32_bluepad32/
│   └── esp32_bluepad32.ino              # ESP32 Arduino 코드 (X/Y/R 전송)
└── ev3_control.py                        # EV3 Pybricks 코드 (메카넘 4륜 믹싱)
```
...
| 특정 바퀴 방향 반대 | 모터 방향 설정 | 해당 모터만 `Direction.COUNTERCLOCKWISE` 로 변경 |
| 전진 시 대각선으로 흐름 | 모터 배치 불일치 | `A/B/C/D` 포트 연결이 표와 같은지 확인 |
